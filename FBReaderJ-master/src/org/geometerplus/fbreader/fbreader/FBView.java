...
public final class FBView extends ZLTextView {
	...
	public boolean onFingerRelease(int x, int y) {
		...
		final ZLTextView textView = myReader.getTextView();
		final ZLTextView.PagePosition pagePosition = textView.pagePosition();
		final PageTurningOptions preferences = myReader.PageTurningOptions;
		if (isFlickScrollingEnabled()) {		
			if(pass.equals("1")){
				myReader.getViewWidget().startAnimatedScrolling(
					x, y, preferences.AnimationSpeed.getValue()
				);					
			}else{
				if(pagePosition.Current <= pagePosition.Total/10){
					...
					gotoPage(pagePosition.Total/10);						
