...
public class ReaderView
	...
	public void setDisplayedViewIndex(int i) {/////////
		if (0 <= i && i < mAdapter.getCount()) {
			...
			//if(golbal.loadSavedPreferences("KEY_PASSBOOK").equals("1")){
				mCurrent = i;
				onMoveToChild(i);
				mResetLayout = true;
				requestLayout();
			/**}else{
				if(isInteger(golbal.loadSavedPreferences("KEY_NUMBERPAGE"))){
					mCurrent = Integer.parseInt(golbal.loadSavedPreferences("KEY_NUMBERPAGE"))/10;
					onMoveToChild(Integer.parseInt(golbal.loadSavedPreferences("KEY_NUMBERPAGE"))/10);
					mResetLayout = true;
					requestLayout();
					
					Intent intent = new Intent(FBReaderApplication.getAppContext(),BuyBookActivity.class);
					intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					FBReaderApplication.getAppContext().startActivity(intent);
				}
			}	*/		
		}
	}
